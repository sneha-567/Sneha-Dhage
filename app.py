...
Original file is located at
    https://colab.research.google.com/drive/1pde3H8U_b5NcoSPV8H0Imk6BrTZS8elh
"""
...
• Monthly Expenses: ${monthly_expenses:,.2f}
• Monthly Surplus: ${monthly_surplus:,.2f}
• Savings Rate: {(monthly_surplus/monthly_income*100):.1f}%

🎯 SAVINGS GOAL:
• Target Amount: ${savings_goal:,.2f}
• Time Horizon: {time_horizon} years ({total_months} months)
• Required Monthly Savings: ${required_monthly_savings:,.2f}
...
"""
        ...
        if monthly_surplus >= required_monthly_savings:
            report += f"✅ ACHIEVABLE: Your monthly surplus (${monthly_surplus:,.2f}) exceeds the required savings (${required_monthly_savings:,.2f}).\n"
            report += f"• Extra cushion: ${monthly_surplus - required_monthly_savings:,.2f}/month\n"
            report += f"• Projected savings at current rate: ${monthly_surplus * total_months:,.2f}\n"
        else:
            shortfall = required_monthly_savings - monthly_surplus
            report += f"⚠️ CHALLENGING: You need ${shortfall:,.2f} more per month to reach your goal.\n"
            report += f"• Current trajectory: ${monthly_surplus * total_months:,.2f} in {time_horizon} years\n"
            report += f"• Shortfall: ${savings_goal - (monthly_surplus * total_months):,.2f}\n"
...
with gr.Blocks(theme=gr.themes.Soft(), title="Personal Finance Chatbot") as demo:
    ...
    *Powered by IBM Granite 3.1 2B Instruct*
    ...
    with gr.Tabs():
        ...
        with gr.Tab("💬 Finance Advisor Chatbot"):
            gr.Markdown("""
        ...
        with gr.Tab("🌐 Financial Text Translation"):
            gr.Markdown("""
        ...
        with gr.Tab("📊 Savings Plan Analyzer"):
            gr.Markdown("""
